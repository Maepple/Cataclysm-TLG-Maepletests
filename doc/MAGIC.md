...
`METABOLISM`            | Multipler that is added after BIO_METABLISM as a percentage. With zero metabolism (`multiply: -1`) doesn't give complete weariness drain immunity.
`BIO_METABOLISM`        | Multiplier for `metabolic_rate_base`, which respond for default bmr rate; Formula for basic bmr is `metabolic_rate_base * ( (weight_in_kg / 10 ) + (6.25 * height) - (5 * age) + 5 )`; Since it's a percent, using `multiply` is recommended; Since metabolism is directly connected to weariness, at this moment decreasing it makes you more weary the less metabolism you have; zero metabolism (`multiply: -1`) is handled separately, and makes you never wear
