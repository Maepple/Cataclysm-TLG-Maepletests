...
`METABOLISM`            | Multipler that is added after BIO_METABLISM as a percentage, does not affect weariness, so zero METABOLISM (`multiply: -1`) doesn't give complete weariness immunity.
`BIO_METABOLISM`        | Multiplier for `metabolic_rate_base`, which affects basal metabolic rate. The formula for BMR is `metabolic_rate_base * ( (weight_in_kg / 10 ) + (6.25 * height) - (5 * age) + 5 )`; Since it's a percent, using `multiply` is recommended; Since metabolism is directly connected to weariness, at this moment decreasing it makes you more weary the less metabolism you have. Complete weariness immunity can be gained with (`multiply: -1`)
