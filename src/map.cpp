...
bool map::is_clingable_wall_adjacent( const tripoint_bub_ms &center ) const
{
    for( const tripoint_bub_ms &p : points_in_radius( center, 1 ) ) {
...
void map::drop_furniture( const tripoint &p )
{
    ...
    while( last_state == SS_NO_SUPPORT ) {
        current.z()--;
        // Check current tile
        last_state = check_tile( current.raw() );
    }

    if( current.raw() == p ) {
        // Nothing happened
        if( last_state != SS_FLOOR ) {
            support_dirty( current.raw() );
        }
...
void map::shoot( const tripoint &p, const tripoint &source, projectile &proj, const bool hit_items,
                 double dispersion )
{
    map::shoot( tripoint_bub_ms( p ), tripoint( source ), proj, hit_items, dispersion );
}

void map::shoot( const tripoint_bub_ms &p, const tripoint &source, projectile &proj,
                 const bool hit_items, double dispersion )
{
    ...
    */
    if( coverage > 0 && coverage < 100 && dispersion < 1000.0 ) {
        float factor = std::sqrt( std::max( 0.0, dispersion ) / 1000.0f );
        coverage *= factor;
    }
    bool hit_something = false;
    // Check again so we can skip if the result was zero.
    if( coverage > 0 || ter( p )->has_flag( ter_furn_flag::TFLAG_HIT_WITHOUT_COVER ) ) {
        int coverage_roll = rng( 1, 99 );
